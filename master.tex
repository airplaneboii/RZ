...
\newtheorem{conseq}[counter]{Condequence}
...
%\begin{equation*}
%    L \in \text{DTime}(T(n)) \equiv \exists \text{deterministic TM M that decides out such that } time_M(n) = \mathscr{O}(T(n))
%\end{equation*}
%
%\begin{equation*}
%    L \in \text{NTime}(T(n)) \equiv \text{ ... nondeterministic ...}
%\end{equation*}
%
%\begin{equation*}
%    P := \Bigcup_{\P: \N \to \N polynomial} DTime(p(n))
%\end{equation*}
%
%
%\begin{equation*}
%    NP := \Bigcup_{\P: \N \to \N polynomial} NTime(p(n))
%\end{equation*}
%
%Proposition: P is c.lsed unter complement.
%
%If $L \in P$ then $\not L \in P$, $\not L = \Sigma^* \setminus L$
%
...
%
%$\complement$ SAT = UNSAT $\Union +$ the set of all words that are not ... of formulas. (Junk)
%
...
%
%\begin{equation*}
%    L \in DSPACE(T(n)) \equiv deterministic TM M that decides for language such that space_M (n) = O(T(n))
%\end{equation*}
%\begin{equation*}
%    L \in NSPACE(T(n)) \equiv nondeterministic TM M that decides for language such that space_M (n) = O(T(n))
%\end{equation*}
%
%PSPACE
%
...
%
%Proof:Suppose $L \in NP$. By the certificate characterisation of NP there exists a deterministic ptime TM M
...
%
%Takes exponential time but only polynomial space. (end of proof)
%
%
...
%
%Proof idea: Look at all possible configurations of a nondeterministic TM that uses only polynomial space.
...
%Theorem: $EXP \neq NEXP => P \neq NP$
%
%Proof: Suppose $L \in NEXP \setminus EXP$. Let $p(n)$ be a polynomial such that $L$ is decided by nondet TM M that runs in $2^{p(n)}$ steps.
...
%    \item For $c, c+$ in the config graph $\Psi("c", "c'") = 1 \equiv $ there is a directed path of length $2^i$ from $c$ to $c'$.
%\end{itemize}
%
%Given this we are done, because the required ptime reduction from $L$ to TQBF is:
%\begin{itemize}
%    \item given the input word $w$, construct $\Psi_{M, w} (u->, v->)$ as above, and hence $\Psi_N(u->, v->)$
%    \item Then idkwhat
%\end{itemize}
...
%\end{itemize}
%
%BPP = $\{L | \text{there exists an expected ptime probabilistic TM that decides L with bounded error}\}$
...
%
%\subsection*{An interactive proof showing that $G_1 /=~ G_2$}
...
%There exists a behaviour of P such that if $G_1 /=~ G_2$ then P(accept) = 1.
%
%\subsection*{Soundness}
%For any P that conforms to the protocol (returns $j \in \{1, 2\}$)
%If $G_1 =~ G_2$ then P(reject) = 1/2
%
%\subsection*{Argument}
...
%
%    \begin{equation*}
%        L \in IP if \exists \text{ an expected polytime probabilistic TM V such that:}
%        (completeness: \exists proover that interacts with V such that \forall w \in L. Pr(Out<v,p,w>=accept) \geq 2/3)
%        (Soundness: \forall proovers P that interact with V and \forall w \notin L Pr(out<v,p,w>=reject) \geq 2/3)
%    \end{equation*}
%\end{definition}
%
...
%Recall
%\begin{equation*}
%    GI = \{"G_1", "G_2" | G_1 =~ G_2 are isomorphic graphs\}
%\end{equation*}
%
...
%Zero-knowledge: If $G_1 =~ G_2$ amd P behaves following the rules then no V* learns anything about $\pi$ from the interaction.
%
...
%$#3-SAT_D = \{("\phi", k) | \phi is a 3-CNF and k = number of satisfying assignmentsfor \phi\}$
%
%\begin{theorem}
%    $#3-SAT_D \in IP$
%\end{theorem}
